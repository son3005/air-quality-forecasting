"""
models/TFT/pipeline.py

TFT Pipeline — Per-Horizon Training with Region-based Evaluation.
Uses the self-contained PyTorch TFT model for PM2.5 forecasting.
"""
...
# TFT model configurations: (d_model, n_heads)
MODEL_CONFIG = {
...
def run():
    print("=" * 70)
    print(f"  TFT Pipeline — Per-Horizon | Block: {BLOCK}")
    ...
    for h in HORIZONS:
        ...
        for r_name, sids in REGIONS.items():
            ...
            best_val = float('inf')
            patience_cnt = 0
            save_dir = os.path.join('models_saved', BLOCK, 'TFT')
    ...
    # ══════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 90)
    print("  FINAL BENCHMARK — TFT (Per-Horizon)")
    print("=" * 90)
