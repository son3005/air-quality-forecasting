...
def print_benchmark_table(results):
    ...
    for i, res in enumerate(results):
        print(f"{i+1:<5} | {'S04_GraphOnly':<15} | {'GCN thuần túy':<20} | {res['in_horizon']:<15} | {res['out_horizon']:<15} | {res['RMSE']:<8.4f} | {res['MAE']:<8.4f} | {res['MAPE']:<8.4f} | {res['R2']:<8.4f} | {res['CSI']:<12.4f} | {res['Score']:<8.4f} | {res['hp']:<30} | {res.get('note', 'OK'):<40}")
    print("-" * 180)
