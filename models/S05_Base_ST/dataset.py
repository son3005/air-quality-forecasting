...
class AirQualityDataset(Dataset):
    def __init__(self, data, seq_len=24, pred_len=24, weather_indices=None):
        self.data = data
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.weather_indices = weather_indices if weather_indices is not None else []
    ...
    def __getitem__(self, idx):
        ...
        if len(self.weather_indices) > 0:
            x_future = self.data[idx + self.seq_len : idx + self.seq_len + self.pred_len, :, self.weather_indices]
            return torch.tensor(x, dtype=torch.float32), torch.tensor(y, dtype=torch.float32), torch.tensor(x_future, dtype=torch.float32)
...
def load_data(normalized_dir='data/normalized'):
    ...
    return X[train_indices], X[val_indices], X[test_indices], num_features, features_list

def get_dataloaders(normalized_dir='data/normalized', seq_len=48, pred_len=24, batch_size=32):
    train_data, val_data, test_data, num_features, features_list = load_data(normalized_dir)
    print(f"[*] Features dimension: {num_features} (pm25 @ index 0)")
    
    weather_cols = ['temp', 'rh', 'precip', 'wind_spd', 'wind_sin', 'wind_cos']
    weather_indices = [features_list.index(col) for col in weather_cols if col in features_list]
    print(f"[*] Future Weather Features found at indices: {weather_indices}")
