"""
models/PatchTST/pipeline.py

PatchTST Pipeline — Per-Horizon Training with Region-based Evaluation.
Uses the Hugging Face PatchTSTForPrediction wrapper for multi-pollutant forecasting.
"""
...
# PatchTST model configurations: (patch_len, stride, d_model, n_heads, e_layers, d_ff)
MODEL_CONFIG = {
...
from model import HFPatchTSTModel
...
def run():
    print("=" * 70)
    print(f"  PatchTST Pipeline — Per-Horizon | Block: {BLOCK}")
    ...
    for h in HORIZONS:
        ...
        for r_name, sids in REGIONS.items():
            ...
            # Model Configuration
            patch_len, stride, d_model, n_heads, e_layers, d_ff = MODEL_CONFIG[h][r_name]
            ...
            best_val = float('inf')
            patience_cnt = 0
            save_dir = os.path.join('models_saved', BLOCK, 'PatchTST')
    ...
    # ══════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 90)
    print("  FINAL BENCHMARK — PatchTST (Per-Horizon)")
    print("=" * 90)
